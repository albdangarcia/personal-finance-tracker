...
export async function fetchSavingGoalsByCategories() {
    noStore();
    try {
        ...
        const categoriesWithTotalContributions = categoriesWithGoals.map(category => {
            const updatedSavingsGoals = category.savingsGoals.map(goal => {
                const totalContributions = goal.contributions.reduce((sum, contribution) => sum + contribution.amount, 0);
                return {
                    ...goal,
                    totalContributions,
                };
...
export async function fetchSavingsGoalById(id: string) {
    noStore();
    try {
